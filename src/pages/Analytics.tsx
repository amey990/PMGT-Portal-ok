...
//   const [exportToast, setExportToast] = useState(false)
//   const [statusProject, setStatusProject] = useState<string>('RRB')

//   const [catTimeline, setCatTimeline] = useState<
//   'today' | 'lastWeek' | '15Days' | '30Days' | '3Months' | '6Months'
// >('today')
...
//       labels: {
//         color: '#fff',
//         boxWidth: 16,        // bigger color square
//         boxHeight: 16,       // bigger color square
...
//                 datalabels:{ display: false }        // ← disable labels here
//               },
//               scales: {
//                 y: { ticks: { stepSize: 5 } }
//               }
...
//  <Card sx={{ bgcolor: '#1C1C1E', borderRadius: 2, height: 300 }}>
//   <CardContent sx={{
//     display: 'flex',
//     flexDirection: 'column',
//     height: '100%',
...
//         plugins={[ChartDataLabels]} // if you’re using chartjs-plugin-datalabels
//       />
//     </Box>
//   </CardContent>
// </Card>
...
// helper for dummy n random ints
const randoms = (n: number) =>
  Array.from({ length: n }, () => Math.floor(5 + Math.random() * 20))

// categories + colors for stacked bar
const categories = [
  'Breakdown',
  'New Installation',
  'Upgrades',
...
export default function Analytics() {
  ...
  const [exportToast, setExportToast]   = useState<boolean>(false)
    ...
    const feProjects = ['RRB','Alpha','Beta','CBI','TCTS']
    const feCounts   = [20, 15, 30, 25, 10]  // dummy FE‐counts per project

    const [feProjectFilter, setFeProjectFilter] = useState<string>('All')
  ...
  // ── KPI Cards State ──
  const [selectedProjectKPI, setSelectedProjectKPI] = useState<string>('RRB')
  const kpis = [
    { label: 'Total Activities', value: 128, icon: ListAltIcon },
    { label: 'Completed',        value:  92, icon: CheckCircleIcon },
    { label: 'Pending',          value:  24, icon: AccessTimeIcon },
  ...
  const handleTimelineChange = (e: SelectChangeEvent<TL>) => {
    ...
    switch(tl) {
      case 'today':
        ...
        break
      case 'lastWeek':
        labels = Array.from({length:7},(_,i)=>format(subDays(today,6-i),'MM/dd'))
        data   = randoms(7)
        break
      case '15Days':
        labels = Array.from({length:15},(_,i)=>format(subDays(today,14-i),'MM/dd'))
        data   = randoms(15)
        break
      case '30Days':
        labels = Array.from({length:6},(_,i)=> {
        ...
        break
      case '3Months':
        labels = Array.from({length:3},(_,i)=>format(subMonths(today,2-i),'MMM yyyy'))
        data   = randoms(3)
        break
      case '6Months':
        labels = Array.from({length:6},(_,i)=>format(subMonths(today,5-i),'MMM yyyy'))
        data   = randoms(6)
        break
    }
  ...
  // ── Stacked Bar (uses its own timeline!) ──
  const [catTimeline, setCatTimeline] = useState<TL>('today')
  ...
  const [stackedData, setStackedData] = useState(buildStackedData('today'))
  const handleCatTimelineChange = (e: SelectChangeEvent<TL>) => {
    const tl = e.target.value as TL
    setCatTimeline(tl)                     // <-- set own state
    setStackedData(buildStackedData(tl))
  }
  ...
  const statusOptions: ChartOptions<'doughnut'> = {
    ...
    plugins: {
      ...
      datalabels: {
        ...
        formatter: (value: number, ctx: any) => {
          const dataArr = ctx.chart.data.datasets[0].data as number[]
          const total   = dataArr.reduce((sum, curr) => sum + curr, 0)
          return Math.round((value/total)*100) + '%'
        }
      },
      tooltip: { enabled: true }
    ...
    <MainLayout title="Analytics" showRightPanel={false}>
      <Box sx={{
        ...
        <Box sx={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:2 }}>
          ...
          {/* Cost vs Duration */}
          <Card sx={{ bgcolor: '#1C1C1E', height: 300, display: 'flex', flexDirection: 'column' }}>
  {/* header row: title + project dropdown */}
  <Box
    sx={{
      display: 'flex',
      alignItems: 'center',
  ...
  >
    ...
    >
      <InputLabel sx={{ color: '#aaa', fontSize: 12 }}>Project</InputLabel>
      <Select
        value={feProjectFilter}
        label="Project"
        onChange={(e) => setFeProjectFilter(e.target.value)}
        sx={{
          color: '#fff',
          height: 32,
          '& .MuiSelect-icon': { color: '#888' },
  ...
  <Box sx={{ flex: 1, px: 2, pb: 2 }}>
    <Bar
      data={{
        labels:
          feProjectFilter === 'All' ? feProjects : [feProjectFilter],
        datasets: [
          {
            label: 'FE Count',
            data:
              feProjectFilter === 'All'
                ? feCounts
                : [feCounts[feProjects.indexOf(feProjectFilter)]],
